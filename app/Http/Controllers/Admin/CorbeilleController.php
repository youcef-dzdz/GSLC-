...
class CorbeilleController extends Controller
{
    ...
    public function restore(Request $request, int $id): JsonResponse
    {
        ...
        if ($modelClass === \App\Models\User::class) {
            $email = $corbeille->snapshot['email'] ?? null;
            if ($email && \App\Models\User::where('email', $email)->exists()) {
                return response()->json([
                    'message' => 'Impossible de restaurer : un utilisateur avec cet email existe déjà. Supprimez ou modifiez l\'utilisateur existant avant de restaurer.'
                ], 422);
            }
        }
