...
<head>
    ...
    <title>GSLC — Réinitialiser le mot de passe</title>
    ...
    <style>
        ...
        * { font-family:'Inter',sans-serif; box-sizing:border-box; margin:0; padding:0; }
        ...
        .input-field { width:100%; border:1.5px solid #e2e8f0; border-radius:10px; font-size:.875rem; background:#f8fafc; color:var(--text-main); transition:border-color .2s,box-shadow .2s,background .2s; outline:none; padding:12px 42px; }
        ...
        .eye-btn { position:absolute; top:50%; transform:translateY(-50%); right:12px; background:none; border:none; cursor:pointer; color:#94A3B8; font-size:.85rem; padding:4px; transition:color .2s; }
...
<div style="display:flex; width:100%; min-height:100vh;">
    ...
    <div class="port-panel hidden lg:flex" style="width:45%; flex-direction:column; position:relative; overflow:hidden; padding:3rem;">
        ...
        <div style="position:relative; z-index:10; flex:1; display:flex; flex-direction:column; justify-content:center;">
            ...
            <p style="color:rgba(255,255,255,0.85); font-size:1.05rem; margin:0 0 4px;">Nouveau mot de passe</p>
    ...
    <div style="flex:1; display:flex; flex-direction:column; justify-content:center; align-items:center; padding:2rem; background:var(--bg-soft); position:relative;">
        ...
        <div class="fade-up" style="width:100%; max-width:420px; background:#fff; padding:2.5rem; border-radius:16px; box-shadow:0 10px 40px rgba(0,0,0,.03); border:1px solid #e2e8f0;">
            ...
            <div style="width:52px; height:52px; background:rgba(16,185,129,.08); border-radius:14px; display:flex; align-items:center; justify-content:center; margin-bottom:1.5rem;">
                <i class="fas fa-shield-alt" style="color:#059669; font-size:1.2rem;"></i>
            </div>

            <h1 class="syne" style="font-size:1.8rem; font-weight:800; color:var(--navy); margin:0 0 8px;">Nouveau mot de passe</h1>
            ...
            {{-- General error --}}
            @if(session('error'))
            <div style="background:#FEF2F2; border:1px solid #FECACA; border-radius:10px; padding:14px 16px; margin-bottom:1.5rem; display:flex; align-items:flex-start; gap:10px;">
            ...
            <form method="POST" action="{{ route('client.password.update') }}" class="fade-up-2">
                @csrf
