...
@section('page_title', $t('Mes demandes'))
...
<div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm border-l-4 border-l-[#CFA030] p-4 mb-5 flex items-center justify-between">
    <div>
        <h1 class="text-2xl font-bold text-[#0D1F3C]">{{ $t('Mes demandes d\'importation') }}</h1>
        <p class="text-sm text-[#6B7280]">{{ $t('Portail client NASHCO') }}</p>
    </div>
    <div class="flex items-center gap-3">
        <a href="{{ route('client.demands.create') }}"
           class="bg-[#CFA030] hover:bg-[#b8881f] text-white rounded-lg px-4 py-2 text-sm font-medium transition-colors flex items-center gap-2">
            <i class="fas fa-plus"></i> {{ $t('Nouvelle demande') }}
        </a>
...
<div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-4 mb-4">
    <form method="GET" action="{{ route('client.demands') }}" class="flex flex-wrap gap-3 items-center">
        ...
        <select name="statut" class="border border-[#CBD5E1] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1A4A8C] bg-white">
            ...
            <option value="{{ $s }}" {{ request('statut') === $s ? 'selected' : '' }}>{{ $t($s) }}</option>
            @endforeach
        </select>
        <button type="submit" class="bg-[#0D1F3C] text-white hover:bg-[#1A4A8C] rounded-lg px-4 py-2 text-sm font-medium transition-colors flex items-center gap-2">
            <i class="fas fa-search"></i> {{ $t('Filtrer') }}
        </button>
        @if(request()->hasAny(['search','statut']))
        <a href="{{ route('client.demands') }}" class="text-[#1A4A8C] text-sm hover:underline"><i class="fas fa-times mr-1"></i>{{ $t('Réinitialiser') }}</a>
        @endif
    </form>
</div>
...
<div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm overflow-hidden">
    <table class="w-full">
        <thead class="bg-[#0D1F3C]">
            <tr>
                <th class="text-left text-xs font-semibold text-white uppercase tracking-wider px-4 py-3">{{ $t('N° Dossier') }}</th>
                <th class="text-left text-xs font-semibold text-white uppercase tracking-wider px-4 py-3">{{ $t('Type') }}</th>
                <th class="text-left text-xs font-semibold text-white uppercase tracking-wider px-4 py-3">{{ $t('Priorité') }}</th>
                <th class="text-left text-xs font-semibold text-white uppercase tracking-wider px-4 py-3">{{ $t('Soumis le') }}</th>
                <th class="text-left text-xs font-semibold text-white uppercase tracking-wider px-4 py-3">{{ $t('Statut') }}</th>
                <th class="text-right text-xs font-semibold text-white uppercase tracking-wider px-4 py-3">{{ $t('Détail') }}</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr class="border-b border-[#E2E8F0] hover:bg-[#EBF4FF] transition-colors">
                ...
                <td class="px-4 py-3">
                    ...
                    <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-bold bg-red-50 text-red-600">{{ $t('URGENTE') }}</span>
                    @else
                    <span class="text-xs text-[#6B7280]">{{ $t('Normale') }}</span>
                    @endif
                </td>
            ...
            <tr>
                <td colspan="6" class="py-16 text-center">
                    ...
                    <p class="text-[#6B7280] font-medium">{{ $t('Aucune demande soumise') }}</p>
                    <a href="{{ route('client.demands.create') }}" class="mt-3 inline-flex items-center gap-2 bg-[#CFA030] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-[#b8881f] transition-colors">
                        <i class="fas fa-plus"></i> {{ $t('Soumettre une demande') }}
                    </a>
                </td>
            </tr>
            @endforelse
