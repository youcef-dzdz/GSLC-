...
<div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm border-l-4 border-l-[#CFA030] p-4 mb-5 flex items-center justify-between">
    <div>
        <h1 class="text-2xl font-bold text-[#0D1F3C]">Bienvenue, {{ $client->raison_sociale }}</h1>
        <p class="text-sm text-[#6B7280]">Portail client NASHCO — {{ $client->type_client }}</p>
    </div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    <a href="{{ route('client.demands') }}"
       class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-4 hover:border-[#1A4A8C] transition-colors group">
        <div class="flex items-center justify-between mb-2">
            <p class="text-xs text-[#6B7280] uppercase tracking-wider">Mes demandes</p>
        ...
        <p class="text-xs text-[#94A3B8] mt-1">Total soumis</p>
    </a>
    ...
    <a href="{{ route('client.invoices') }}"
       class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-4 hover:border-amber-300 transition-colors group">
        <div class="flex items-center justify-between mb-2">
            <p class="text-xs text-[#6B7280] uppercase tracking-wider">Factures ouvertes</p>
...
<div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-5 mb-4">
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-sm font-semibold text-[#0D1F3C] flex items-center gap-2">
            <i class="fas fa-clock text-[#CFA030]"></i> Dernières demandes
        </h3>
        <a href="{{ route('client.demands') }}" class="text-xs text-[#1A4A8C] hover:underline">Tout voir</a>
    </div>

    @if($recentDemands->isEmpty())
    <p class="text-sm text-[#94A3B8] text-center py-6">Aucune demande soumise</p>
    @else
    <div class="space-y-2">
        ...
        <a href="{{ route('client.demands.show', $demand->id) }}"
           class="flex items-center gap-3 p-3 rounded-lg hover:bg-[#F0F4F8] transition-colors border border-transparent hover:border-[#E2E8F0]">
            ...
            <span class="text-xs px-2 py-0.5 rounded-full {{ $statusColor }} flex-shrink-0">{{ str_replace('_', ' ', $demand->statut) }}</span>
            @if($demand->priorite === 'URGENTE')
            <span class="text-xs bg-red-50 text-red-600 px-1.5 py-0.5 rounded font-semibold flex-shrink-0">URGENT</span>
            @endif
        </a>
        @endforeach
    </div>
...
<div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-5">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        @foreach([
            ['route' => 'client.demands.create', 'icon' => 'fa-plus-circle',    'label' => 'Nouvelle demande',  'color' => 'text-[#CFA030] bg-amber-50'],
            ['route' => 'client.quotes',         'icon' => 'fa-file-alt',       'label' => 'Mes devis',         'color' => 'text-blue-600 bg-blue-50'],
            ['route' => 'client.invoices',       'icon' => 'fa-file-invoice',   'label' => 'Mes factures',      'color' => 'text-amber-500 bg-amber-50'],
            ['route' => 'client.containers',     'icon' => 'fa-box',            'label' => 'Mes conteneurs',    'color' => 'text-[#1A4A8C] bg-[#EBF4FF]'],
        ] as $link)
