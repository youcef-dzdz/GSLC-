...
export const TerminauxTab = ({ canEdit }: { canEdit: boolean }) => {
  ...
  // Build port options only from ports that actually have terminaux (avoids phantom seeded ports)
  const portOptions = useMemo(() =>
    Array.from(
      new Map(terms.map(term => [term.port?.id, term.port]).filter(([id]) => id)).values()
    ) as Port[],
  [terms]);
              ...
              <tr key={term.id} className="hover:bg-[#F9FBFF] transition-colors">
                ...
                <td className={C.td}>
                  <div className="flex gap-1">
                    <button onClick={() => openEdit(term)} className="p-1.5 rounded-lg hover:bg-[#EDF4FF] text-[#3A5A8A] hover:text-[#0D2A5E] transition-colors" title={t('common.edit')}><Pencil size={13} /></button>
                    <button onClick={() => setConfirm(term.id)} className="p-1.5 rounded-lg hover:bg-[#FFF0F0] text-[#8A2020] hover:text-[#5A0000] transition-colors" title={t('common.delete')}><Trash2 size={13} /></button>
                  </div>
                </td>
              </tr>
            )) : (
