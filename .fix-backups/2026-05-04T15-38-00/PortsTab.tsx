          ...
          <tbody>
            {filtered.length > 0 ? filtered.map(p => (
              <tr key={p.id} className="hover:bg-[#F9FBFF] transition-colors">
                ...
                <td className={C.td}><span className={`px-2 py-0.5 rounded text-[10px] font-bold ${TYPE_PORT[p.type_port].cls}`}>{TYPE_PORT[p.type_port].label}</span></td>
                ...
                <td className={C.td}>
                  <div className="flex gap-1">
                    {canEdit && <button onClick={() => openEdit(p)} className="p-1.5 rounded-lg hover:bg-[#EDF4FF] text-[#3A5A8A] hover:text-[#0D2A5E] transition-colors" title={t('common.edit')}><Pencil size={13} /></button>}
