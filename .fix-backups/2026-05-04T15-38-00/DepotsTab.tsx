          ...
          <tbody>
            {filtered.length > 0 ? filtered.map(d => (
              <tr key={d.id} className="hover:bg-[#F9FBFF] transition-colors">
                ...
                <td className={C.td}><span className={`px-2 py-0.5 rounded text-[10px] font-bold ${TYPE_STOCKAGE[d.type_stockage].cls}`}>{TYPE_STOCKAGE[d.type_stockage].label}</span></td>
                ...
                <td className={C.td}>
                  <div className="flex gap-1">
                    {canEdit && <button onClick={() => openEdit(d)} className="p-1.5 rounded-lg hover:bg-[#EDF4FF] text-[#3A5A8A] hover:text-[#0D2A5E] transition-colors" title={t('common.edit')}><Pencil size={13} /></button>}
                    {canEdit && <button onClick={() => setConfirm(d.id)} className="p-1.5 rounded-lg hover:bg-[#FFF0F0] text-[#8A2020] hover:text-[#5A0000] transition-colors" title={t('common.delete')}><Trash2 size={13} /></button>}
                  </div>
                </td>
              </tr>
            )) : (
