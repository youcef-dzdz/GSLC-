...
import { Pencil, Trash2, Plus, X, Anchor, Search } from 'lucide-react';
...
const C = {
  ...
  fi:    'border border-[#C5D8F5] rounded-lg px-3 py-2 text-sm text-[#0D2A5E] focus:outline-none focus:border-[#C8960A]',
  label: 'block text-xs font-semibold text-[#3A5A8A] mb-1',
  reset: 'border border-[#C5D8F5] text-[#3A5A8A] text-sm px-3 py-2 rounded-lg hover:bg-[#EDF4FF]',
};
...
const PortsTab = ({ canEdit }: { canEdit: boolean }) => {
  ...
  const filtered = ports.filter(p => {
    ...
    const matchSearch = !q || p.nom_port.toLowerCase().includes(q) || p.code_port.toLowerCase().includes(q) || p.ville.toLowerCase().includes(q);
    ...
    <>
      ...
      <div className="flex flex-wrap gap-2 mb-3 items-center">
        <div className="relative flex-1 min-w-[180px]">
          <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#88A8D0]" />
          <input className={C.fi + ' pl-8 w-full'} placeholder="Rechercher un port..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        ...
        <button onClick={() => { setSearch(''); setFilterType(''); }} className={C.reset}>Réinitialiser</button>
        <button onClick={openNew} className="ml-auto flex items-center gap-2 px-4 py-2 text-sm font-semibold bg-[#C8960A] text-white rounded-xl hover:bg-[#A87A08]"><Plus size={14} />{t('admin.ports.new')}</button>
      </div>
      ...
      <div className="bg-white rounded-xl border border-[#C5D8F5] overflow-hidden">
        {isLoading ? <Skel /> : ports.length === 0 ? (
          ...
          <div className="py-12 text-center text-[#88A8D0] text-sm">Aucun résultat pour ces filtres.</div>
        ) : (
                ...
                <tr key={p.id} className="hover:bg-[#F4F9FF] transition-colors">
                  ...
                  <td className={C.td}>
                    <div className="flex gap-1">
                      {canEdit && <button onClick={() => openEdit(p)} className="p-1.5 rounded-lg hover:bg-[#EDF4FF] text-[#3A5A8A] hover:text-[#0D2A5E]"><Pencil size={13} /></button>}
                      {canEdit && <button onClick={() => setConfirm(p.id)} className="p-1.5 rounded-lg hover:bg-[#FFF0F0] text-[#8A2020] hover:text-[#5A0000]"><Trash2 size={13} /></button>}
                    </div>
                  </td>
                </tr>
              ))}
...
const TerminauxTab = ({ canEdit }: { canEdit: boolean }) => {
  ...
  const filtered = terminaux.filter(term => {
    const q = search.toLowerCase();
    const matchSearch = !q || term.nom_terminal.toLowerCase().includes(q) || term.code_terminal.toLowerCase().includes(q);
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap gap-2 mb-3 items-center">
        <div className="relative flex-1 min-w-[180px]">
          <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#88A8D0]" />
          <input className={C.fi + ' pl-8 w-full'} placeholder="Rechercher un terminal..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        <select className={C.fi} value={filterPort} onChange={e => setFilterPort(e.target.value)}>
          <option value="">Tous les ports</option>
          {(ports as Port[]).map(p => <option key={p.id} value={String(p.id)}>{p.nom_port}</option>)}
        </select>
        <button onClick={() => { setSearch(''); setFilterPort(''); }} className={C.reset}>Réinitialiser</button>
        <button onClick={openNew} className="ml-auto flex items-center gap-2 px-4 py-2 text-sm font-semibold bg-[#C8960A] text-white rounded-xl hover:bg-[#A87A08]"><Plus size={14} />{t('admin.terminaux.new')}</button>
      </div>
            ...
            <tbody className="divide-y divide-[#EEF5FF]">
              {filtered.map(term => (
                <tr key={term.id} className="hover:bg-[#F4F9FF] transition-colors">
                  ...
                  <td className={C.td}>
                    <div className="flex gap-1">
                      {canEdit && <button onClick={() => openEdit(term)} className="p-1.5 rounded-lg hover:bg-[#EDF4FF] text-[#3A5A8A] hover:text-[#0D2A5E]"><Pencil size={13} /></button>}
                      {canEdit && <button onClick={() => setConfirm(term.id)} className="p-1.5 rounded-lg hover:bg-[#FFF0F0] text-[#8A2020] hover:text-[#5A0000]"><Trash2 size={13} /></button>}
                    </div>
                  </td>
                </tr>
              ))}
...
const DepotsTab = ({ canEdit }: { canEdit: boolean }) => {
  ...
  const filtered = depots.filter(d => {
    const q = search.toLowerCase();
    const matchSearch = !q || d.code_depot.toLowerCase().includes(q) || d.nom_depot.toLowerCase().includes(q);
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap gap-2 mb-3 items-center">
        <div className="relative flex-1 min-w-[180px]">
          <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#88A8D0]" />
          <input className={C.fi + ' pl-8 w-full'} placeholder="Rechercher un dépôt..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        <select className={C.fi} value={filterType} onChange={e => setFilterType(e.target.value)}>
          <option value="">Tous les types</option>
        ...
        <button onClick={() => { setSearch(''); setFilterType(''); setFilterPort(''); }} className={C.reset}>Réinitialiser</button>
        <button onClick={openNew} className="ml-auto flex items-center gap-2 px-4 py-2 text-sm font-semibold bg-[#C8960A] text-white rounded-xl hover:bg-[#A87A08]"><Plus size={14} />{t('admin.depots.new')}</button>
      </div>
            ...
            <tbody className="divide-y divide-[#EEF5FF]">
              {filtered.map(d => (
                <tr key={d.id} className="hover:bg-[#F4F9FF] transition-colors">
                  ...
                  <td className={C.td}>
                    <div className="flex gap-1">
                      {canEdit && <button onClick={() => openEdit(d)} className="p-1.5 rounded-lg hover:bg-[#EDF4FF] text-[#3A5A8A] hover:text-[#0D2A5E]"><Pencil size={13} /></button>}
                      {canEdit && <button onClick={() => setConfirm(d.id)} className="p-1.5 rounded-lg hover:bg-[#FFF0F0] text-[#8A2020] hover:text-[#5A0000]"><Trash2 size={13} /></button>}
                    </div>
                  </td>
                </tr>
              ))}
