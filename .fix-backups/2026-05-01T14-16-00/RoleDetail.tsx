    ...
    <div className="flex flex-col min-h-full bg-[#F8FAFC]" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-white border-b border-[#E2E8F0] px-6 py-4"
        ...
        <button
          ...
          className="flex items-center gap-2 text-white/70 hover:text-white text-sm font-medium mb-3 cursor-pointer transition-colors"
        >
          <ArrowLeft size={16} />
          {tlx('back')}
        </button>
        <div className="flex items-center gap-3 flex-wrap">
          <h1 className="text-xl font-bold text-white">{role.nom_role}</h1>
          <span className="font-mono text-xs bg-white/20 text-white px-2 py-0.5 rounded">{role.label}</span>
          <span className="text-xs font-bold px-2 py-0.5 rounded" style={{ backgroundColor: nv.bg, color: nv.text }}>N{role.niveau}</span>
          {role.is_system ? (
            <span className="inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded bg-white/20 text-white">
              <Lock size={10} />{tlx('system')}
            </span>
          ) : (
            <span className="text-xs font-semibold text-white/60 bg-white/10 px-2 py-0.5 rounded">{tlx('custom')}</span>
          )}
        </div>
        {role.description && <p className="text-white/60 text-sm mt-1">{role.description}</p>}
      </div>
          ...
          Object.entries(grouped).map(([module, perms]) => {
            ...
            return (
              <div key={module} className="bg-white rounded-xl border border-[#E2E8F0] shadow-sm mb-4 overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3" style={{ background: '#0D1F3C' }}>
