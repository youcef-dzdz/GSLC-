...
const Modal = ({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-4">
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
      <div className="flex items-center justify-between px-6 py-4 border-b border-[#C5D8F5]">
  ...
  return (
    <>
      <div className="flex justify-end mb-3">
        {canEdit && <button onClick={openNew} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold bg-[#C8960A] text-white rounded-xl hover:bg-[#A87A08]"><Plus size={14} />{t('admin.ports.new')}</button>}
      </div>
...
const TerminauxTab = ({ canEdit }: { canEdit: boolean }) => {
  ...
  return (
    <>
      <div className="flex justify-end mb-3">
        {canEdit && <button onClick={openNew} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold bg-[#C8960A] text-white rounded-xl hover:bg-[#A87A08]"><Plus size={14} />{t('admin.terminaux.new')}</button>}
      </div>
      <div className="bg-white rounded-xl border border-[#C5D8F5] overflow-hidden">
...
const DepotsTab = ({ canEdit }: { canEdit: boolean }) => {
  ...
  return (
    <>
      <div className="flex justify-end mb-3">
        {canEdit && <button onClick={openNew} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold bg-[#C8960A] text-white rounded-xl hover:bg-[#A87A08]"><Plus size={14} />{t('admin.depots.new')}</button>}
      </div>
      <div className="bg-white rounded-xl border border-[#C5D8F5] overflow-hidden">
      ...
      <div className="flex gap-1 mb-4 bg-white border border-[#C5D8F5] rounded-xl p-1 w-fit">
        {tabs.map(tb => (
          <button
            ...
            className={`px-4 py-1.5 text-sm font-semibold rounded-lg transition-all ${tab === tb.key ? 'bg-[#0D2A5E] text-white' : 'text-[#3A5A8A] hover:bg-[#EDF4FF]'}`}
          >
            {tb.label}
          </button>
        ))}
